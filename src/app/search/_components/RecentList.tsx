  ...
  return (
    <section>
      <div className="mb-3 flex items-center justify-between">
        ...
        <button onClick={() => setRecentSearches([])} className="text-text-secondary text-xs">
          전체삭제
        </button>
      </div>
      <ul className="flex flex-col gap-3">
        {recentSearches.map((item) => (
          <li key={item} className="flex items-center gap-4">
            <button className="flex items-center gap-2" onClick={() => setQuery(item)}>
              <Clock className="text-icon-quaternary h-4 w-4 shrink-0" />

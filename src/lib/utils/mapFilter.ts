...
/**
 * 지도 핀 클라이언트 필터.
 * 핀 유형(전체/명소/동네)과 개화 상태는 서버 파라미터가 없어 응답을 받아 여기서 거른다.
 * 핀 하나에 꽃이 여러 개일 수 있으므로 statuses 는 "하나라도 해당" 이면 통과시킨다.
 */
export function filterMapSpots(spots: MapSpot[], filter: MapSpotFilter): MapSpot[] {
  const { pinType, statuses } = filter
  // 거를 조건이 없으면 같은 배열을 그대로 돌려줘 불필요한 재렌더를 막는다.
  if (pinType === 'ALL' && statuses.length === 0) return spots

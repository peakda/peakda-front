...
| 5 | P2 | 필터 조건 파라미터 부재 | **대폭 축소** — 1건만 남음 |
...
### 5. `GET /api/search/spots` 에 `category` 파라미터가 없습니다 *(대폭 축소)*

초판에서 요청드린 필터 파라미터 대부분은 프론트에서 해결했습니다. **남은 건 이 한 줄입니다.**
...
- **`GET /api/explore` 의 `region`** — 철회. 지역 6개 권역은 프론트가 임의로 만든 값이었고, 서버 기준 코드도 없어 **필터 드로어의 지역 탭 자체를 제거**했습니다. 나중에 서버 기준 권역 코드가 생기면 그때 다시 논의하겠습니다.

...
카카오·네이버는 연결됐지만 애플은 핸들러가 없다. `disabled` 도 아니라 정상 버튼으로 보이고, 누르면 아무 일도 일어나지 않는다. **첫 화면에서 사용자가 가장 먼저 만지는 지점**이라 체감이 크다.

2026-08-20 백엔드 확인: **애플 로그인은 지원하지 않는다. 추후 구글 로그인으로 교체될 예정**이라 `/oauth2/authorization/apple` 자체가 열리지 않는다 — 카카오·네이버처럼 3줄로 끝나는 문제가 아니라, 백엔드에 엔드포인트가 생길 때까지 프런트만으로 고칠 수 없다.

- 지금 당장은: `disabled` + "준비 중" 표기로 최소 방어 (또는 구글 대체 일정이 잡히면 버튼 자체를 교체)
- 구글로 교체될 때: 카카오·네이버와 같은 패턴(`window.location.href` 로 백엔드 도메인 직행)일 가능성이 높다 (`src/lib/auth/socialLogin.ts` 참고)
